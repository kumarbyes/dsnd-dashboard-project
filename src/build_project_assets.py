...
notes = pd.DataFrame(_, columns=['employee_id', 'employee_name', 'note']).\
                    assign(event_date=np.random.choice(df.event_date, size=len(_), replace=True))  # noqa: E501
...
y = X.join(df.drop_duplicates('employee_id').set_index('employee_id')[['recruited']]).recruited  ## noqa: E501
